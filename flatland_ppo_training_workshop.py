...
def parse_args():
    ...
    parser.add_argument("--use-pretrained-network", type=bool, default=False, 
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            print('rollout data before update: {}'.format(rollout_data[step]['done']))
            rollout_data[
                step
            ].update_(next_obs)# save for training, also includes the done
            print('rollout data after update: {}'.format(rollout_data[step]['done']))
            # ALGO LOGIC: action logic
            with torch.no_grad():
                print('value before network: {}'.format(rollout_data[step]['values']))
                rollout_data[[step]] = (td_module(rollout_data[[step]]))
                print('value after network: {}'.format(rollout_data[step]['values']))
            # rollout_data[[step]] = env.step(rollout_data[step]).to(device) doesn't work because we would overwrite observations for step
            next_obs, rewards= env.step(rollout_data[step])
            print('rewards before update: {}'.format(rollout_data[step]['rewards']))
            rollout_data[step].update_(rewards) # save the rewards received for actions in current step
            print('rewards after update: {}'.format(rollout_data[step]['rewards']))
            
            if next_obs['done']:
                print('next obs before done: {}'.format(next_obs))
                next_obs.update_(env.reset()) # we are not overwriting the true done
                print('next obs after done: {}'.format(next_obs))
                if args.do_render:
                    env_renderer.reset()
