...
class RailEnv_td(RailEnv):
        ...
        obs_td = TensorDict(
            {
                "agents_attr": torch.tensor(
                    observations[0], dtype=torch.float32
                ),  # .unsqueeze(0),
                "node_attr": torch.tensor(
                    observations[1][0], dtype=torch.float32
                ),  # .unsqueeze(0),
                "adjacency": torch.tensor(
                    observations[1][1], dtype=torch.int64
                ),  # .unsqueeze(0),
                "node_order": torch.tensor(
                    observations[1][2], dtype=torch.int64
                ),  # .unsqueeze(0),
                "edge_order": torch.tensor(
                    observations[1][3], dtype=torch.int64
                ),  # .unsqueeze(0)
            },
            [self.get_num_agents()],
        )
    ...
    def step(self, tensordict):
        # print('doing step')
        actions = {
            handle: action.item() for handle, action in enumerate(tensordict["actions"])
        }
        ...
        tensordict_out = TensorDict({}, batch_size=[])
        tensordict_out["observations"] = self.obs_to_td(observations)
        tensordict_out["rewards"] = torch.tensor(
        ...
        # Agents not done (arrival_time is None)
            ...
            if agent.state.is_off_map_state():
                reward = (
                    ...
                    * (
                        agent.get_travel_time_on_shortest_path(self.distance_map)
...
if __name__ == "__main__":
    ...
    # print('td params: {}'.format(td_module.parameters()))
    ...
    observations = TensorDict(
        {
            "agents_attr": torch.zeros(num_steps, num_agents, 83, dtype=torch.float32),
            "node_attr": torch.zeros(
                num_steps, num_agents, 31, 12, dtype=torch.float32
            ),
            ...
            "valid_actions": torch.zeros(
                num_steps, num_agents, num_actions, dtype=torch.bool
            ),
        },
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, num_steps):
            ...
            rollout_data[
                step
            ] = next_obs  # something goes wrong when we save the rollout data
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                logratio = (
                    updated_rollout_data["logprobs"] - rollout_data["logprobs"][mb_inds]
                )
                ratio = logratio.exp()
                ...
                # optimizer.zero_grad()
                # loss.backward()
                nn.utils.clip_grad_norm_(td_module.parameters(), args.max_grad_norm)
                # optimizer.step()
                # print('current_loss: {}'.format(loss))
                print("sum of weights: {}".format(sum(list(td_module.parameters())[0])))

            if args.target_kl is not None:
                if approx_kl > args.target_kl:
                    break
        # writer.add_scalar('weights/random trial weight', sum(list(td_module.parameters().flatten())))
        # print('weights: {}'.format(list(td_module.parameters())))
